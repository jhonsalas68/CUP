<div class="space-y-6">
    ...
    <div class="bg-white dark:bg-zinc-900 p-4 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-xs flex items-center gap-2">
        <div x-data="voiceSearchWidget($wire)" class="relative flex items-center gap-2 grow">
            ...
            <div class="grow">
                <flux:input wire:model.live.debounce.300ms="search" placeholder="Buscar por nombre, email, CI o especialidad..." icon="magnifying-glass" class="w-full" />
            </div>
                            ...
                            <flux:table.cell>
                                <div class="flex items-center gap-3">
                                    <div class="w-9 h-9 rounded-xl bg-indigo-100 dark:bg-indigo-950/50 text-indigo-700 dark:text-indigo-300 font-bold text-sm flex items-center justify-center shrink-0">
                                        {{ strtoupper(substr($docente->nombre ?? $docente->user?->name ?? 'D', 0, 1)) }}
                                    </div>
                                    <div class="min-w-0">
                                        <p class="font-semibold text-zinc-900 dark:text-zinc-100 truncate">{{ $docente->nombre ?? $docente->user?->name ?? '—' }}</p>
                                    </div>
                                </div>
                            </flux:table.cell>
                            <flux:table.cell class="text-sm text-zinc-650 dark:text-zinc-400">{{ $docente->user?->email ?? '—' }}</flux:table.cell>
                            <flux:table.cell class="text-sm text-zinc-650 dark:text-zinc-400">{{ $docente->ci }}</flux:table.cell>
                            <flux:table.cell class="text-sm text-zinc-650 dark:text-zinc-400">{{ $docente->especialidad ?? '—' }}</flux:table.cell>
                            <flux:table.cell class="text-sm text-zinc-650 dark:text-zinc-400">{{ $docente->telefono ?? '—' }}</flux:table.cell>
    ...
    <!-- Modal -->
    <flux:modal name="docente-modal" wire:model.self="showModal" class="w-full max-w-lg">
        <div class="space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <flux:field class="sm:col-span-2">
                    ...
                    <flux:textarea wire:model="formacion_academica" placeholder="Licenciatura en..." rows="2" />
                    <flux:error name="formacion_academica" />
                </flux:field>
                <div class="sm:col-span-2 space-y-2.5 mt-2 border-t border-zinc-150 dark:border-zinc-800 pt-3">
                    <flux:label class="block font-semibold text-zinc-800 dark:text-zinc-200">Requisitos de Contratación (FICCT)</flux:label>
                    <div class="flex flex-col gap-2">
                        <label class="inline-flex items-center gap-2 text-sm text-zinc-700 dark:text-zinc-300 cursor-pointer select-none">
                            <input type="checkbox" wire:model="profesional_area" class="rounded border-zinc-350 dark:border-zinc-700 text-indigo-650 focus:ring-indigo-500 bg-white dark:bg-zinc-900">
                            <span>Es profesional en el área</span>
                        </label>
                        <label class="inline-flex items-center gap-2 text-sm text-zinc-700 dark:text-zinc-300 cursor-pointer select-none">
                            <input type="checkbox" wire:model="tiene_maestria" class="rounded border-zinc-350 dark:border-zinc-700 text-indigo-650 focus:ring-indigo-500 bg-white dark:bg-zinc-900">
                            <span>Tiene Maestría</span>
                        </label>
                        <label class="inline-flex items-center gap-2 text-sm text-zinc-700 dark:text-zinc-300 cursor-pointer select-none">
                            <input type="checkbox" wire:model="tiene_diplomado" class="rounded border-zinc-350 dark:border-zinc-700 text-indigo-650 focus:ring-indigo-500 bg-white dark:bg-zinc-900">
                            <span>Tiene Diplomado en Educación Superior</span>
                        </label>
                    </div>
                </div>
            </div>
